...
export function Feed({ updates, students }: FeedProps) {
  ...
  return (
    <div className="pb-24 pt-4 px-4 max-w-md mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight mb-1">{t('feedTitle')}</h1>
        <p className="text-sm text-gray-500 font-medium">{t('feedSubtitle')}</p>
      </div>
            ...
            <div key={update.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{update.authorName}</p>
                    <p className="text-xs text-gray-500">{date} {student ? `• ${t('about')} ${student.name}` : ''}</p>
                  </div>
                </div>
                <button 
